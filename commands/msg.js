...
class Command{
	...
	run(core, bot, user, args, payload){
		...
		if (!trip || !message) return bot.reply(payload, '抱歉，参数错误，请检查后再试')
		if (!/^[a-zA-Z0-9/\+]{6}$/.test(trip)) return bot.reply(payload, '你管这叫识别码？')
		this.manager.add(trip, user.nick, user.trip || '', message)
		bot.reply(payload, `我记下来了，当识别码为 ${trip} 的用户说话的时候，我会提醒TA查看的`)
