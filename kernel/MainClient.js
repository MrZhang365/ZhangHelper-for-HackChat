...
class UserList {
	...
	add(payload) {
		if (payload.trip === this.core.config.admin) payload.level = 3    // 超级管理员
...
class MainClient extends WebSocket {
	...
	async handleMessage(data) {
		...
		try{
			payload = this.runHooks(JSON.parse(data))
		} catch (err) {
			return this.core.logger.error(`Failed to run hook, error: \n${err}`)
		}
		...
		if (cmd === 'onlineSet') {
			...
			if (!user) {
				...
				this.reply(payload ,`很抱歉，我无法从我的数据中找到你的信息`)
				return
			}
			const cmdResult = await this.core.commands.handleCommand(user, payload)
			if (cmdResult === 404) {
				this.reply(payload ,`抱歉，这好像不在我的工作范围之内。`)
				return
			}
			if (cmdResult === 403) {
				this.reply(payload, '你是想篡权夺位么？')
				return
			}
			if (cmdResult !== false){ 
				this.reply(payload, '抱歉，我在处理你的请求时，出现了未知问题。')
				return
			}
			...
			if (payload.type === 'whisper' && typeof payload.from === 'string') {
				const nick = payload.from
				const text = payload.text.slice(nick.length + 12)
				const user = this.users.get(nick)
				payload.whisper = true    // 告诉命令，这个是私信
				payload.text = text
	...
	reply(payload, text) {
		if (payload.whisper) {
			this.whisper(payload.from, text)
		} else {
			this.chat(`@${payload.nick} ${text}`)
		}
	}
	whisper(nick, text) {
		this.sendJSON({
