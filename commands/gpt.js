...
class Command {
	...
	async run(core, bot, user, args, payload){
		if (!user.trip) return bot.whisper(user.nick, `我是不会为一个没有识别码的用户服务的`)
		if (args.length < 2) return bot.whisper(user.nick, `你想问什么？`)

		const question = args.slice(1).join(' ')
		const result = await this.gpt(core, user, question)

		if (result === false) {
			return bot.whisper(user.nick, `我出错了`)
		}
		
		bot.whisper(user.nick, result)
	}
}
